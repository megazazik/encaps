...
function ic2(c) {
    var s = c.getInitState();
    var actions = c.getActions();
    actions.a1(2534);
    actions.a2('345345');
}
function ib2(b) {
    var c = b.setInitState(function () { return ({ p1: 'sdf', p2: '876873', p3: 453656 }); })
        .action({
        add: function (state, action) { return state; },
        remove: function (state, action) { return state; }
    })
        .subAction({
        list: function (state, actions) { return state; }
    })
        .getController();
    var s = c.getInitState();
    var a = c.getActions();
}
//# sourceMappingURL=controllerOld.js.map