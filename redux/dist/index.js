...
var encaps_react_1 = require("encaps-react");
function connect(_a) {
    ...
    var getDispatch = function (dispatch) {
        if (!cachedDispatch) {
            cachedDispatch = path ? encaps_1.wrapDispatch(path, dispatch) : dispatch;
        }
        return cachedDispatch;
    };
    var getChildState = function (state) { return encaps_1.getStatePart(path, state); };
...
function connectView(params) {
    return function (component, path) {
        ...
        var componentParams = pathParts.reduce(function (prevParams, pathPart) { return encaps_react_1.wrapConnectParams(pathPart, prevParams); }, params);
        var connectParams = encaps_react_1.composeConnectParams(encaps_react_1.parentConnectParams, componentParams);
